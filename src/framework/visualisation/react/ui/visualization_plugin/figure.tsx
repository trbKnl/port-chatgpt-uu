...
import { VisualizationData, ChartVisualizationData, TextVisualizationData } from "./types";
...
import { zTable, zVisualizationData, zVisualizationType } from "./types";
import { z } from "zod";
...
export const Figure = ({
  ...
  if (tableValidator.success === false || visualizationValidator.success === false) {
    if (tableValidator.success === false) console.error(tableValidator.error);
    if (visualizationValidator.success === false) console.error(visualizationValidator.error);
    return <div></div>;
  }
...
export const FigureComponent = ({
  ...
  if (visualizationData == null && status === "loading") return <Loader />;
  if (status === "error") {
...
function useValidatedInput(tableInput: any, visualizationInput: any) {
  // tableInput and visualizationInput are not typesafe because this would require
  // that the visualization plugin also includes the Python dataclasses. Instead,
  // we validate the input here.
  const table = useMemo(() => {
    const tableValidator = zTable.safeParse(tableInput);
    if (!tableValidator.success) {
      console.error(tableValidator.error);
      return undefined;
    }
    return tableValidator.data;
  }, [tableInput]);
  const visualization = useMemo(() => {
    const visualizationValidator = zVisualizationData.safeParse(visualizationInput);
    if (!visualizationValidator.success) {
      console.error(visualizationValidator.error);
      return undefined;
    }
    return visualizationValidator.data;
  }, [visualizationInput]);
